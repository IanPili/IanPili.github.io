...
var wInitScroll = function() {
    ...
    icId.addEventListener('wheel', wheelType);
    icId.addEventListener('mousedown', wHoldScroll);
    icId.addEventListener('touchstart', wHoldScroll);

    // icId.addEventListener('mouseover', pauseAuto);
    // icId.addEventListener('mouseout', resumeAuto);
  ...
  function wheelType(e){
    ...
    wScrollProperties.isAuto = false;
    wScrollMove(e);
  }
    ...
    if(wScrollProperties.initialize) {
        ...
        if(wScrollProperties.lastScrollY < wScrollProperties.scrollDirection) {
            ...
            if(wScrollConfig.barDown >= wScrollProperties.newBarY) {
                ...
                // console.log('scroll:', wScrollProperties.scrollDirection);
                // console.log('scroll22:', wScrollProperties.lastScrollY);
        ...
        if(canMove) {
            ...
            if(!wScrollProperties.isAuto){
                ...
                w(wScrollConfig.barId).changeStyle('transform;transition',  'translate(' + wScrollConfig.barX + 'px, ' + wScrollProperties.newBarY + 'px);transform 0s linear');
                w(wScrollConfig.contentId).changeStyle('transform;transition',  'translate(' + wScrollConfig.contentX + 'px, ' + wScrollProperties.newContentY + 'px);transform 0s linear');

            }else{

                w(wScrollConfig.barId).changeStyle('transform;transition',  'translate(' + wScrollConfig.barX + 'px, ' + wScrollProperties.newBarY + 'px) rotate(0deg);transform .59s linear');
                w(wScrollConfig.contentId).changeStyle('transform;transition',  'translate(' + wScrollConfig.contentX + 'px, ' + wScrollProperties.newContentY + 'px) rotate(0deg);transform .59s linear');
            }
        }
    }
}
